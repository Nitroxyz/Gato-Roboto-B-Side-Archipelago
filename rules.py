...
def set_all_location_rules(world: GatoRobotoWorld) -> None:
    ...
    if world.options.loresanity:
        ...
        current_location = world.get_location("Main Terminal (Nexus-1615)")
        set_rule(current_location, lambda state: True)
        current_location = world.get_location("Secret Terminal Dev (Nexus-1610)")
        set_rule(current_location, lambda state: (state.has("Spin Jump", world.player) and state.has("Dash", world.player) and state.has("Hopper", world.player)))
        current_location = world.get_location("Secret Terminal Anna (Nexus-2109)")
        set_rule(current_location, lambda state: state.has("Rocket", world.player))
        ...
        set_rule(current_location, lambda state: True)
        current_location = world.get_location("Secret Terminal (Win)")
        set_rule(current_location, lambda state: state.has("Dash", world.player) and state.has("Rocket", world.player))
