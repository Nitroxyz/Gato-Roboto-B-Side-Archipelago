...
def set_all_location_rules(world: GatoRobotoWorld) -> None:
    ...
    if world.options.loresanity:
        ...
        set_rule(current_location, lambda state: (state.has("Vent Level", world.player, 3) and state.has("Rocket", world.player)) or (state.has("Vent Level", world.player, 3) and state.has("Spin Jump", world.player)) or ((world.options.gato_tech == 3)))
